 ...
 * mencatat transaksi Midtrans Rp 0 yang tidak pernah terjadi.
 */
class VoucherRedeemer
{
     ...
     * lewat Midtrans, pembatalannya urusan panitia, bukan tombol di admin.
     */
    public function release(VoucherRedemption $redemption): void
    {
        DB::transaction(function () use ($redemption): void {
            ...
            if (! $registration->isWaived() && $registration->status === 'paid') {
                $this->fail(
                    'Registrasi ini sudah dibayar sebagian lewat Midtrans. Lepas slotnya secara manual bersama panitia keuangan.',
                    'This registration was partly paid through Midtrans. Release the slot together with the finance team.',
                );
            }
