...
<div class="grid gap-4 sm:grid-cols-3">
    @foreach(['abstract' => ['Abstrak', 'Abstract'], 'payment' => ['Pembayaran', 'Payment'], 'full_paper' => ['Full paper', 'Full paper']] as $kind => $labels)
        ...
        <div class="rounded-xl border border-slate-200 bg-white p-5">
            ...
            <p class="mt-2 text-sm text-slate-600">{{ $date ? $date->translatedFormat('d M Y, H:i').' WITA (UTC+8)' : (__('site.deadline_to_be_announced')) }}</p>
            @if($date && $date->isPast())<p class="mt-2 text-xs font-semibold text-red-700">{{ __('site.deadline_closed') }}</p>@endif
        </div>
    @endforeach
</div>
