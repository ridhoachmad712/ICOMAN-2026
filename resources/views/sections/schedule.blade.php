...
<section class="py-16">
    <div class="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
            ...
            <div class="divide-y divide-slate-100 rounded-xl border border-slate-200 bg-white">
                @foreach($items as $item)
                    <div class="flex flex-col gap-1 px-5 py-4 sm:flex-row sm:items-baseline sm:gap-5">
                        <span class="shrink-0 text-sm font-semibold text-[var(--brand)]">{{ $item->time_start }}@if($item->time_end) – {{ $item->time_end }}@endif</span>
                        <div>
        ...
        <div class="mt-8 text-center">
            <a href="{{ route('program') }}" class="text-[var(--brand)] font-medium hover:underline">{{ __('site.view_all') }} →</a>
        </div>
    </div>
</section>
