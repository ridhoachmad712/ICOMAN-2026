...
- **Invoice yatim**: `registrations.submission_id` memakai nullOnDelete, jadi paper yang dihapus lalu dikirim ulang meninggalkan invoice tanpa paper — dan invoice seperti itu tidak akan pernah menampilkan pilihan SINTA 3. `RegistrationProvisioner` kini mengambil alih invoice semacam itu saat checkout berikutnya (kecuali yang sudah lunas). Data lama: `php artisan icoman:relink-invoices --fix`. Diagnosa satu invoice: `php artisan icoman:diagnose-invoice <id>`.
...
- **Rekap keuangan**: `Submission → Rekap Keuangan` (`FinanceReport`). Ringkasan (uang masuk, piutang, dibebaskan voucher, menunggu verifikasi), pemasukan per kategori tarif, dan daftar piutang + export CSV. Hitungannya milik `App\Support\FinanceSummary`, satu tempat untuk semua tampilan. Dua kaidahnya: pemasukan dihitung dari baris pembayaran berhasil (bukan status invoice), dan piutang hanya dari invoice yang masih mungkin dibayar (yang dibebaskan voucher dan yang gagal dikeluarkan).

- **Peran `bendahara`**: mengurus uang saja. Melihat Transaksi dan Registrations (termasuk Tandai Lunas/Tolak), plus statistik Registrasi & Pembayaran di dashboard. TIDAK punya: Submissions, tarif, voucher, co-host, akun author, users, Pengaturan. Aturannya `User::handlesMoney()`. Matriks peran di `AdminRoleMatrixTest` mengikat semuanya.
- **`payments.recorded_by`**: setiap pencatatan manual meninggalkan nama penekannya; pembayaran gateway kosong karena memang tidak ada orang di baliknya.
...
- **Pergantian gateway**: order berstatus `initiated` milik gateway lama tidak ikut berpindah, dan pemakaian ulang order antar-tab akan menyerahkan kembali tautan checkout lama — author membayar ke tempat yang statusnya tidak pernah kita tanyakan lagi. Order dari gateway selain yang sedang dipakai kini tidak pernah dipakai ulang. Data lama: `php artisan icoman:retire-old-payments --fix` (pembayaran yang sudah lunas tidak disentuh).
